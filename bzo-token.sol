...
contract CarbonToken is ERC20, Ownable, ERC20Detailed {
    address constant bezonianWallet = 0xA4e611EDaE26c9689C853cD224f0051741595538;               // Bezonian (Master Wallet)
    ...
    constructor() public ERC20Detailed("BEZON", "BZO", 9) {
        totalCoins = 1000000000 * 10**uint256(decimals());
        _mint(owner(), totalCoins); // total supply fixed at 1 billion tokens
        ERC20.transfer(presalesWallet, 50000000 * 10**uint256(decimals())); // transfer 50 million tokens
        ERC20.transfer(privateSalesWallet, 25000000 * 10**uint256(decimals())); // transfer 25 million tokens
    }
    ...
    /**
     * @dev transfer of token to Bezonian Address.
     * always require the sender has enough balance
     * @return the bool true if success.
     * @param amount The amount to be transferred.
     */
    ...
    {
        require(amount <= getAvailableBalance(msg.sender));
        return ERC20.transfer(bezonianWallet, amount);
    }
